...
class Cmsmart_Megamenu_Block_Navigation extends Mage_Catalog_Block_Navigation
{
        ...
        $isOutermost = false, $outermostItemClass = '', $childrenWrapClass = '', $noEventAttributes = false)
    {
        ...
        if ($level == 0) {
            $widthMenu = Mage::getStoreConfig('megamenu/mainmenu/width_menu') - 20;
            $html[] = '<ul class="level' . $level .' '. $addclass .' column'.$this->getLevle($id).'" style="width:'.$widthMenu. 'px;">';
        } else {
    ...
    /**
     * 
     * @param integer $id Identification.
     * @return integer
     */
    ...
    public function getShowblockleft($active, $id, $cid) 
    {
        if ($active == 1) {
            ...
            if (is_array($width)) {
                if (array_key_exists('auto_width', $width[0])) {
    ...
    public function widthp($idc)
    {
        ...
        if (($width[0]['active_static_block'] == 1 )&& ($width[0]['active_static_block_left'] == 1 ) && ($width[0]['active_static_block_right'] == 1 )) {
            $widthchildrenli = $widthMenu -  $width[0]['width_block_left'] -  $width[0]['width_block_right']-30-10;
        } else {
            if (($width[0]['active_static_block'] == 1 )&& ($width[0]['active_static_block_left'] == 1 ) && ($width[0]['active_static_block_right'] == 0 )) {
                $widthchildrenli = $widthMenu -  $width[0]['width_block_left']-(30*$column)-30;
            } else {
                if (($width[0]['active_static_block'] == 1 )&& ($width[0]['active_static_block_left'] == 0 ) && ($width[0]['active_static_block_right'] == 1 )) {
                    $widthchildrenli = $widthMenu -  $width[0]['width_block_right']-30;
                } else {
                    $widthchildrenli = $widthMenu ;
                }
            }
        }
